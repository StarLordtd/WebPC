...
function phanTich_URL_chiTietSanPham() {
  ...
  s += addThongSo("Mainboard", sanPhamHienTai.detail.mainboard);
  s += addThongSo("Ram", sanPhamHienTai.detail.ram);
  s += addThongSo("Ổ cứng", sanPhamHienTai.detail.ocung);
  s += addThongSo("VGA", sanPhamHienTai.detail.vga);
  s += addThongSo("Nguồn máy tính", sanPhamHienTai.detail.nguonmaytinh);
  s += addThongSo("Vỏ case", sanPhamHienTai.detail.vocase);
  s += addThongSo("Tản nhiệt", sanPhamHienTai.detail.tannhiet);
  s += addThongSo("Chú thích", sanPhamHienTai.detail.chuthich);
  info.innerHTML = s;
...
function suggestion() {
  ...
  const sanPhamTuongTu = list_products
    ...
    .map((sanPham) => {
      ...
      let cungHangSanXuat = sanPham.loaiPC === sanPhamHienTai.loaiPC;

      // Tiêu chí 4: cùng loại khuyến mãi
      let cungLoaiHang = sanPham.promo?.name === sanPhamHienTai.promo?.name;
