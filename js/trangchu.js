window.onload = function () {
  ...
  if (filters.length) {
    ...
    // Các màu
    var he = ["#fff00f", "#0000ff"];
    var gm = ["#ff0000", "#000000"];
    var lv = ["#5de272", "#007012"];
    ...
    //   div
    // );
    // addKhungSanPham("Chuột", yellow_red, ["promo=giamgia"], soLuong, div);
    // addKhungSanPham(
    //   "Tai nghe",
    //   green,
  ...
  for (var i = 0; i < filters.length; i++) {
    ...
    switch (dauBang[0]) {
      ...
      case "loaiPC":
        ...
        if (saveFilter) filtersFromUrl.loaiPC = dauBang[1];
        break;
      ...
      case "promo":
        result = timKiemTheoHang(result, dauBang[1]);
...
function timKiemTheoCongTySanXuat(list, tenCongTy, soluong) {
  ...
  for (var i = 0; i < list.length; i++) {
    if (list[i].loaiPC.toUpperCase().indexOf(tenCongTy.toUpperCase()) >= 0) {
      result.push(list[i]);
      count--;
      if (count <= 0) break;
    }
...
function timKiemTheoHang(list, tenHang, soluong) {
  var count,
    result = [];
  if (soluong < list.length) count = soluong;
  else count = list.length;
...
function addAllChoosedFilter() {
  ...
  if (filtersFromUrl.loaiPC != "")
    addChoosedFilter("loaiPC", filtersFromUrl.loaiPC);
...
function addCompany(img, nameCompany) {
  var link = createLinkFilter("add", "loaiPC", nameCompany);
