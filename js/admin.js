...
function addThongKe() {
  ...
  danhSachDonHang.forEach((donHang) => {
    ...
    donHang.sp.forEach((sanPhamTrongDonHang) => {
      let tenHang = sanPhamTrongDonHang.sanPham.loaiPC;
...
function autoMaSanPham(loaiPC) {
  // hàm tự tạo mã cho sản phẩm mới
  if (!loaiPC) loaiPC = document.getElementsByName("chonloaiPC")[0].value;
  var index = 0;
  for (var i = 0; i < list_products.length; i++) {
    if (list_products[i].loaiPC == loaiPC) {
      index++;
    }
  }
  document.getElementById("maspThem").value = loaiPC.substring(0, 3) + index;
}
...
function addKhungSuaSanPham(masp) {
  ...
  s +=
    ...
    `"></td>
        ...
        <tr>
            <td>Nguồn máy tính:</td>
            <td><input type="text" value="` +
    sp.detail.nguonmaytinh +
    `"></td>
        </tr>
        <tr>
            <td>Vỏ case:</td>
      ...
      if (u[i].donhang[j].ngaymua == maDonHang) {
        if (duyetDon) {
          if (u[i].donhang[j].tinhTrang == "Đang chờ xử lý") {
            u[i].donhang[j].tinhTrang = "Đã xác nhận đơn hàng";
          ...
          } else if (u[i].donhang[j].tinhTrang == "Đang xác nhận đơn hàng") {
